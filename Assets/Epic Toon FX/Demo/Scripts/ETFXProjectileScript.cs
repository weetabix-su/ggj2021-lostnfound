...
public class ETFXProjectileScript : MonoBehaviour
{
    ...
    public float explosiveRadius = 0.15f;
    ...
    void FixedUpdate()
    {
        ...
        {
            transform.rotation = Quaternion.LookRotation(rg.velocity); // Sets rotation to look at direction of movement
        }
        ...
        Vector3 direction = rg.velocity; // Gets the direction of the projectile, used for collision detection
        if (rg.useGravity)
            direction += Physics.gravity * Time.deltaTime; // Accounts for gravity if enabled
        direction = direction.normalized;

        float detectionDistance = rg.velocity.magnitude * Time.deltaTime; // Distance of collision detection for this frame
